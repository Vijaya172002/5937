...
public class CrudRoleService {
	@Autowired
	JdbcTemplate jt;
public int insert(CrudRole employee) {
	...
	if((loginrole.equals("H"))||loginrole.equals("A")){
		...
		if(emprole!="") {
			...
			return jt.update(sql2,employee.getId(),employee.getFname(),employee.getLname(),email,employee.getDob(),employee.getDept(),emprole,employee.getPhoneno(),uname,password,employee.getSalary());
		}
