...
@Service
public class ProjectEmpServ {

		@Autowired
		JdbcTemplate jt;
		public String inst(Project employee) {
			...
			if(roleData1.isEmpty()) {
				...
				for(Map fetchdata:roleData1) {
					...
					int phoneno1=employee.getPhoneno();
				...
				int ins=jt.update(sql2,id,fname,email,phoneno,employee.getProjid(),loginuname,empjoindate,Projassigneddate);
